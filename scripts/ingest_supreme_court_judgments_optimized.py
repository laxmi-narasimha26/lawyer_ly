#!/usr/bin/env python3
"""
Optimized Supreme Court Judgments Ingestion Script
Processes downloaded Supreme Court judgments with optimized chunking based on data analysis
"""
...
class OptimizedTextProcessor:
    """Optimized text processor based on Supreme Court data analysis"""
...
class SupremeCourtIngestionPipeline:
    """
    Optimized pipeline for processing Supreme Court judgments
    """
    ...
    async def process_judgment(self, pdf_content: bytes, metadata: Dict[str, Any], year: int) -> bool:
        """Process a single Supreme Court judgment with optimized chunking"""
        try:
            ...
            logger.info(f"  Created {len(chunks)} optimized chunks for judgment {judgment_data['case_id']}")
            ...
            for i, chunk in enumerate(chunks):
                ...
                chunk_data = {
                    ...
                    "chunk_index": i,
                    "chunk_type": "content",
                    "token_count": int(len(chunk.split()) * 1.3),  # Rough estimate
                    "metadata": json.dumps({
            ...
            if processed_chunks > 0:
                avg_chunk_size = total_chunk_size / processed_chunks
                self.stats["avg_chunk_size"] = (self.stats["avg_chunk_size"] * (self.stats["processed_judgments"] - 1) + avg_chunk_size) / self.stats["processed_judgments"]
                self.stats["avg_chunks_per_judgment"] = self.stats["total_chunks"] / self.stats["processed_judgments"]
            
            logger.info(f"✅ Processed judgment: {judgment_data['case_id']} ({processed_chunks} chunks, avg size: {total_chunk_size//processed_chunks if processed_chunks > 0 else 0} chars)")
            return True
    ...
    async def process_year_data(self, year: int, max_judgments: Optional[int] = None) -> bool:
        ...
        try:
            ...
            with zipfile.ZipFile(english_zip, 'r') as eng_zip:
                ...
                logger.info(f"Processing {len(pdf_files)} PDF files with optimized chunking...")
                ...
                for pdf_file in pdf_files:
                    try:
                        ...
                        if processed_count % 5 == 0:
                            ...
                            logger.info(f"  Avg chunks per judgment: {self.stats['avg_chunks_per_judgment']:.1f}")
                            logger.info(f"  Avg chunk size: {self.stats['avg_chunk_size']:.0f} chars")
                        
                        # Rate limiting to avoid overwhelming OpenAI API
                        if processed_count % 3 == 0:
                            await asyncio.sleep(1)  # Brief pause every 3 judgments
            ...
            logger.info(f"📊 Average chunks per judgment: {self.stats['avg_chunks_per_judgment']:.1f}")
            logger.info(f"📊 Average chunk size: {self.stats['avg_chunk_size']:.0f} characters")
            return True
    ...
    def get_ingestion_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_metrics": {
                "avg_chunks_per_judgment": self.stats["avg_chunks_per_judgment"],
                "avg_chunk_size": self.stats["avg_chunk_size"],
                "chunking_strategy": "hierarchical_legal_structure_aware"
            }
        }
    ...
    def save_ingestion_summary(self):
        ...
        summary_file = self.data_dir / "optimized_ingestion_summary.json"
        
        with open(summary_file, 'w') as f:
            json.dump(summary, f, indent=2)
        
        logger.info(f"📄 Optimized ingestion summary saved to: {summary_file}")

async def main():
    """Main function with command line interface"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Optimized Supreme Court Judgments Ingestion")
    ...
    try:
        ...
        if args.year:
            ...
            pipeline.setup_database()
            available_years = []
            for year_dir in pipeline.data_dir.iterdir():
            ...
            for year in available_years:
                year_success = await pipeline.process_year_data(year, args.max_judgments)
        ...
        if success:
            logger.info("🎉 Optimized ingestion completed successfully!")
            summary = pipeline.get_ingestion_summary()
            logger.info(f"📊 Processed {summary['stats']['processed_judgments']} judgments")
            logger.info(f"📊 Created {summary['stats']['total_chunks']} optimized chunks")
            logger.info(f"📊 Success rate: {summary['success_rate']:.1f}%")
            logger.info(f"📊 Avg chunks per judgment: {summary['optimization_metrics']['avg_chunks_per_judgment']:.1f}")
            logger.info(f"📊 Avg chunk size: {summary['optimization_metrics']['avg_chunk_size']:.0f} chars")
        else:
